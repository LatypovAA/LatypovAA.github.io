...
class Send_mail {
        ...
        {      
            if(!isset($this->_params[$name])){
                $this->_error_text .= 'Invalid option! '.$name.'()<br>';
                $this->_error = false;
            }else if(count($param) > 1){
                $this->_error_text .=  '1 parameter expected'.$name.'()!<br>';
                $this->_error = false;
            }else{
        ...
        private function _error_email()
        {
            if(empty($this->_params['email'])){
                $this->_error = false;
                $this->_error_text .= 'Unknown option: email ()<br>';       
            }

            if(empty($this->_params['from_mail'])){
                $this->_error = false;
                $this->_error_text .= 'Unknown option: from_mail()<br>';   
            }
...
//                return mail($email, $subject, $message, $header);
                ...
                return mail($this->_params['email'], $subject, $message, $header);
            }      
        }      
}